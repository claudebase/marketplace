...
3. Summarize results in a compact brief:
   ```
   📦 Summary:
     - Code: src/devkit (42 files), pm/ (TypeScript agents)
   ...
   ```
4. If regeneration is needed, instruct the devkit Agent to run the automated index task or execute it via available tools.

Keep responses short and data-driven so the devkit Agent can reference the brief without rereading the entire repository.
